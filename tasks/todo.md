...
- [ ] P3-3 视觉/话术特征 MVP
  - 视频帧质量、人脸/姿态、ASR、话术结构后置到一期采集稳定后。
...
2. `P0-5 评论补采命中率诊断` 已完成，当前主要瓶颈是 `493` 条待补目标尚无评论产物。
...
- 评论失败诊断显示，评论补采首要问题不是少量空响应，而是大量 `missing_artifact`，下一批应优先稳定跑完待补队列，再针对 `noise_only` 优化接口监听过滤。
